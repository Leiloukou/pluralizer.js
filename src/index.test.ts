...
describe('#plural-old', () => {
	...
	it('will NOT pluralize "ostrich" because there is no number before it', () => {
		expect(pluralOld`${['s']}I only have ${'an'} ostrich${_FP}, sorry.`).toBe(
...
describe('#plural-new', () => {
	...
	it('will NOT pluralize "ostrich" because there is no number before it', () => {
		expect(
			plural(['s'])`I only have ${'an'} ostrich${_FP}, sorry.`
...
describe('#plural-swap', () => {
	it('should pluralize "gram".', () => {
		...
		expect(`I have ${data} ${pluralSwap("gram", "grams", data)} of sugar in this cup of coffee.`)
	})

	it('should NOT pluralize "sandwich".', () => {
		const data = 1;

		expect(`I have ${data} ${pluralSwap("sandwich", "sandwiches", data)} left, would you like half?`)
	})
})
